        ...
        if (($CurrentState.values | Where-Object { $_.name -eq 'EnableGroupCreation' }).value -eq 'false') {
            ...
            try {
                if (!$CurrentState) {
                    ...
                    New-GraphPostRequest -tenantid $tenant -Uri "https://graph.microsoft.com/v1.0/settings/$($CurrentState.id)" -AsApp $true -Type POST -Body $CurrentState -ContentType 'application/json'
                    $CurrentState = (New-GraphGetRequest -Uri 'https://graph.microsoft.com/v1.0/settings' -tenantid $tenant) | Where-Object -Property displayname -EQ 'Group.unified'
                }
