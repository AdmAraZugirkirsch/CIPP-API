...
function Invoke-ExecGDAPInvite {
    ...
    switch ($Action) {
        'Create' {
            ...
            $OnlySOC = ($NumberOfRoles -eq 2 -and $RoleMappings.RoleName -contains "Security Operator" -and $RoleMappings.RoleName -eq "Security Administrator")
            ...
            try {
                ...
                if ($CheckActive.status -eq 'created') {
                    ...
                    Write-LogMessage -headers $Request.Headers -API $APINAME -message "Test $DisplayName" -Sev 'Info'
